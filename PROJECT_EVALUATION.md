...
## 十一、第十一轮优化归档（2026-08-07）

> 执行范围：A 组（易用性 5 项）+ B 组（生产增强 8 项）+ C 组（工程化 6 项），共 19 项。
> 执行环境：conda t2xpu，Python 3.13，Flask 3.1.2，flask-smorest 0.47.0，pytest 9.1.1。

### 11.1 关键发现（执行过程中新发现）

| # | 位置 | 问题 | 修复 |
|---|------|------|------|
| F1 | `requirements.txt` | **flask-smorest>=1.2.0,<2.0 永远无法安装**：PyPI 上该包最高版本为 0.47.0（无 1.x），pip install 直接失败 | 改为 `>=0.42.0,<1.0`（requirements.txt + README） |
| F2 | `flask_server/module/sqlalchemy.py` | `sqlalchemy_trans` 装饰器在 app context 外调用时报错 "Working outside of application context"，业务函数需手动创建 context | 装饰器自动创建 context：`has_app_context()` 判断 + `_app.app_context()` 包裹，业务函数内无需手动 with |

### 11.2 A 组：易用性（5 项）

| # | 修复内容 | 说明 |
|---|---------|------|
| A1 | `.env` 自动加载 | 引入 python-dotenv，`config.py` 自动加载项目根 `.env`，README 教程"cp .env.example .env"即可生效 |
| A2 | Redis 去掉每次 ping | 正常时零探测直通（操作成功即证明可用），失败进入 30s 冷却期，冷却结束后首次操作 ping 恢复探测。避免每次 get/set 多一次 RTT |
| A3 | request_id 可观测性 | 响应回写 `X-Request-Id` 头（透传或自动生成）；`LOG_FORMAT=json` 时日志附带 `request_id` 字段，ELK 可聚合 |
| A4 | examples 修复 | 补全结构 + `__init__.py`，controller 仅引用 examples 内路径；service 延迟导入 + 未配置占位模型；`random_string` 改 `secrets_token`；反射式改声明式 `article_declared.py`，补 BuyRecordPO |
| A5 | README 同步 | 测试覆盖列表、422 FAQ、.env 说明等更新 |

### 11.3 B 组：生产增强（8 项）

| # | 修复内容 | 说明 |
|---|---------|------|
| B1 | 限流组件 | `component/rate_limit.py`：按 (IP, 路径) 固定窗口计数（memory_cache TTL），`RATE_LIMIT_ENABLED=false` 默认关，`RATE_LIMIT_PER_MINUTE=60`，超出 429 |
| B2 | 可信代理 | `TRUSTED_PROXIES`，默认 `127.0.0.1,::1`；`get_real_ip` 仅在请求来自可信代理时信任 X-Forwarded-For，防 IP 伪造 |
| B3 | webui 缓存有界 | `path_exist_cache` 改 OrderedDict + 锁 + 上限 2048，LRU 淘汰，修复无界增长内存泄漏 |
| B4 | 线程池有界 | `BoundedExecutor`（信号量 + 上限 `ASYNC_TASK_QUEUE_MAX=500`），满时拒绝并告警，修复无界队列内存膨胀 |
| B5 | 健康检查增强 | docker-compose app 服务加 healthcheck（GET /api/v1/health）；`/health` 返回 `version`/`uptime` |
| B6 | DB 集成测试 | 新增 3 文件：`sqlalchemy_trans` 提交/回滚 + 用户 CRUD、sqlite 临时库 + 声明式 model、SQLite 真实库 CRUD + LIMIT 校验 |
| B7 | 安全响应头 | X-Content-Type-Options / X-Frame-Options / Referrer-Policy / 宽松 CSP（/docs 除外），`SECURITY_HEADERS_ENABLED=true` 默认开 |
| B8 | UTC 时间工具 | `DateTimeUtil.utc_now_str()` / `format_timestamp_utc()`，不再依赖服务器本地时区 |

### 11.4 C 组：工程化（6 项）

| # | 修复内容 |
|---|---------|
| C1 | GitHub Actions CI：Python 3.10/3.12 矩阵 + pytest |
| C2 | pyproject.toml：元信息 + pytest 配置 + ruff 配置段，支持 `pip install -e .` |
| C3 | 配置文件补充注释（grace_result / common / config / redis_cache） |
| C4 | docker-compose.prod.yml：waitress 入口、SECRET_KEY 必填校验、日志挂载、不暴露 DB/Redis 端口 |
| C5 | 本文档评估轮次归档 |
| C6 | README 补 flask-smorest ETag/Page 分页用法 |

### 11.5 测试数量验证

- 用例总数 **61 → 108**，新增 47 个用例全部通过
- 冒烟验证：`import flask_server` / `import wsgi` 均可正常导入，无副作用污染
- 集成测试：HTTP 集成（统一响应/422/404/request_id 头/安全头/限流/Redis 降级恢复/BoundedExecutor/DB 事务集成/UTC 时间）
